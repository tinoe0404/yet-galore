...
import { adminStaggerContainer, adminCardFadeUp } from '@/lib/animations';

interface DashboardClientProps {
  stats: { totalProducts: number; featuredProducts: number; unreadEnquiries: number; totalCategories: number };
  recentProducts: any[];
  recentEnquiries: any[];
}

export function DashboardClient({ stats, recentProducts, recentEnquiries }: DashboardClientProps) {
  return (
            ...
            <table className="w-full text-left font-sans text-sm">
              ...
              <tbody className="divide-y divide-border">
                ...
                {recentProducts.length === 0 && (
                  <tr><td colSpan={3} className="px-6 py-12 text-center text-muted">No products yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
            ...
            <table className="w-full text-left font-sans text-sm">
              ...
              <tbody className="divide-y divide-border">
                ...
                {recentEnquiries.length === 0 && (
                  <tr><td colSpan={3} className="px-6 py-12 text-center text-muted">No enquiries yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function StatCard({ title, value, highlight }: { title: string; value: number; highlight?: boolean }) {
  return (
    <motion.div
      variants={adminCardFadeUp}
      className={`p-6 bg-white border ${highlight ? 'border-red-400 shadow-sm' : 'border-border'}`}
    >
